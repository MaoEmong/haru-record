...
아직 앱 이름은 확정하지 않았습니다. 이 프로젝트는 사용자가 별도로 기록하지 않아도 기기 안에서 위치 흐름을 모으고, 하루 단위로 이동 경로와 머문 곳을 정리해 돌아볼 수 있게 만드는 Flutter 앱입니다.
...
- 오늘: 현재 기록 중인 하루를 음악 플레이어처럼 보여주는 메인 화면입니다.
- 오늘 기록: 오늘 쌓인 위치 기록, 이동 경로, 방문 기록을 자세히 봅니다.
- 흐름보기: 하루의 좌표 흐름을 재생바와 함께 시간순으로 봅니다.
- 돌아보기: 생성된 하루 기록을 날짜별로 모아봅니다.
- 하루 상세: 특정 날짜의 요약, 이동 경로, 방문 기록을 봅니다.
- 방문한 곳: 사용자가 저장한 장소와 방문 횟수를 관리합니다.
- 설정: 기록 기준, 권한, 알림, 데이터 관리를 조정합니다.
...
- 전체 화면의 깨진 한글 문구 정리
- 앱 이름, 아이콘, 릴리즈 패키지 정보 확정
- 지도 제공 방식과 타일 정책 검토

## 개발 메모

- 앱 이름은 아직 확정하지 않았으므로 README에서도 특정 이름을 사용하지 않습니다.
